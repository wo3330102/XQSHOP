...
export default {
    ...
    watch:{
        'param.title':function(title){
            if(!this.isTitleFirst && isJSON(title)) this.setTemplateDesign({textDto:this.param})
            if(this.isTitleFirst) this.isTitleFirst=false
        },
        'param.content':function(conetnt){
            if(!this.isTitleFirst && isJSON(conetnt)) this.setTemplateDesign({textDto:this.param})
